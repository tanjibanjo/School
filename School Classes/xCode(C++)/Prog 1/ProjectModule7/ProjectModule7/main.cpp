...
/*     <23 Jan 24> - Added Welcome Header and prompt for user's name
       ...
       <11 Apr 24> - Add option 7 to calculate grades to menu and switch
              - Create getGrades function to prompt and store grades
              - Create displayGrades function to display grades
              - Create averageGrades function to calculate and
                 return average of the grades
       <24 Apr 24> - Create Class Contacts to store the contacts and link
                      to main
              - Create array of instances of Contacts called
                 <myContacts>
              - Populate array in main
              - Option 8 added to menu to display contacts
*/
//************************************************************************
...
int main()
{
    ...
    contacts[0].setLastName("Smith");
    contacts[0].setPhoneNumber("210-856-2435");
    
    contacts[1].setFirstName("Jane");
    contacts[1].setLastName("Smith");
    contacts[1].setPhoneNumber("211-754-3478");
    
    contacts[2].setFirstName("April");
    contacts[2].setLastName("Henricks");
    contacts[2].setPhoneNumber("855-997-7986");
    ...
    while(procedure != 0) //while user does not choose to exit
    {
        ...
        switch(procedure)
        {
            ...
            case 6:
                ...
                break;
            //7 will prompt for 5 grades, display grades and average
            case 7:
                //call function to get grades
                getGrades(gradesArray, ARRAY_SIZE);
                //call function to display grades
                displayGrades(gradesArray, ARRAY_SIZE);
                //call funcction to calculate average and display
                cout << "The average grade is "
                     << averageGrades(gradesArray, ARRAY_SIZE) << ".\n";
                break;
            //8 will call function to display contacts
            case 8:
                cout << "Contact List:\n";
                for(int i = 0; i < 3; i++)
...
//function to prompt user to enter 5 grades
void getGrades(double grades[], int size)
{
    //prompt user to enter in 5 grades
...
{
    for(int i = 0; i < size; i++) //iterate through array to display
    {
        cout << "Grade " << i + 1 << ": " << grades[i] << endl;
    }
}

//function that takes array and returns the average
double averageGrades(double grades[], int size)
{
    //local variables
    double total = 0;
    
    //iterate through array to add to total
